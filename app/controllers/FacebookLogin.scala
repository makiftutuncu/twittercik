...
object FacebookLogin extends Controller
{
  ...
  def getUserInfo[T](accesstoken: String)(implicit request: Request[T]): Future[Response] = WS.url(s"https://graph.facebook.com/me?access_token=$accesstoken").get()
  ...
  def facebookLogin = Action.async { implicit request =>
    Application.getSessionForRequest(request) match {
      case Some(session: Session) =>
        println(s"FacebookLogin.facebookLogin() - User named ${session.username} is already logged in. Redirecting to timeline...")
        Future.successful(Redirect(routes.Timeline.renderPage()))
      case None =>
        if(!request.queryString.contains("code")) {
          ...
          Future.successful(showLoginDialog)
        }
        else {
